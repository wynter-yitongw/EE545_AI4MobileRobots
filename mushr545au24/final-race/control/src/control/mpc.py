...
class ModelPredictiveController(BaseController):
    ...
    def sample_controls(self):
        ...
        controls[:, :, 1] = np.linspace(self.min_delta, self.max_delta, self.K)[:, None]
...
class ModelPredictiveController(BaseController):
    ...
    # Edited: obstacle_size_factor=1.2
    def check_collisions_in_map(self, poses, obstacle_size_factor=1.2, car_size_factor=1.9):
        """Compute which poses are in collision with pixels in the map.
        ...
        # !!!!!!!Edited
        car_bbox = self.car_bbox * car_size_factor
        self.bbox_map = (
            np.matmul(self.car_bbox[np.newaxis, ...], rot) + points[:, np.newaxis]
        )
        # Rounding is about 10x more expensive then a truncating cast.
        ...
        # !!!!!!!Edited
        obstacle_map = binary_dilation(self.obstacle_map, structure=np.ones((int(obstacle_size_factor),int(obstacle_size_factor))))
        self.collisions = self.obstacle_map[bbox_idx[..., 1], bbox_idx[..., 0]].max(
            axis=1
        )
